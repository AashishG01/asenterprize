...
const EnclosedArchitecture = ({ children, floorTex, dk, hl, lt, isPatio = false }: any) => {
  // Dimensions: 5m x 4.5m x 3m tall
  return (
    <group>
      {/* Floor */}
      ...
      {/* Left Wall (Neutral Microcement) x = -2.5 */}
      <mesh position={[-2.5, 1.5, 0]} rotation={[0, Math.PI / 2, 0]} receiveShadow castShadow>
        <planeGeometry args={[4.5, 3]} />
        <meshStandardMaterial color={isPatio ? "#8B7355" : "#e8e6e1"} roughness={0.8} />
      </mesh>

      {/* Right Window Wall (30% solid, 70% glass cutout) x = 2.5 */}
      {!isPatio ? (
        <group position={[2.5, 1.5, 0]} rotation={[0, -Math.PI / 2, 0]}>
          ...
          <mesh position={[-2, 0, 0]} receiveShadow castShadow>
...
export function SceneLivingRoom(props: any) {
  return (
    <EnclosedArchitecture {...props}>
      {/* Large Sofa (scaled up) */}
      <RoundedBox args={[3.5, 0.9, 1.2]} position={[0, 0.45, 0]} radius={0.15} castShadow receiveShadow>
        <meshStandardMaterial color="#e8e5df" roughness={0.9} />
      </RoundedBox>
      <RoundedBox args={[3.5, 0.9, 0.6]} position={[0, 1.35, -0.3]} radius={0.15} castShadow receiveShadow>
        <meshStandardMaterial color="#e8e5df" roughness={0.9} />
      </RoundedBox>
      
      {/* Large Coffee Table */}
      <RoundedBox args={[2.2, 0.35, 1.2]} position={[0, 0.175, 1.5]} radius={0.05} castShadow receiveShadow>
...
export function SceneBathroom(props: any) {
  return (
    <EnclosedArchitecture {...props}>
      ...
      <RoundedBox args={[3.5, 0.6, 1]} position={[0, 1, -1.7]} radius={0.02} castShadow receiveShadow>
        <meshStandardMaterial color="#5c4033" roughness={0.6} />
      </RoundedBox>
      
      {/* White Ceramic Sink */}
      <RoundedBox args={[1.8, 0.15, 0.8]} position={[0, 1.375, -1.6]} radius={0.05} castShadow>
      ...
      <rectAreaLight width={3.5} height={0.2} color="#ffeedd" intensity={5} position={[0, 0.7, -1.7]} rotation={[-Math.PI/2, 0, 0]} />
    </EnclosedArchitecture>
  );
}
...
export function SceneKitchen(props: any) {
  return (
    <EnclosedArchitecture {...props}>
      ...
      <RoundedBox args={[3.8, 1.1, 1.5]} position={[0, 0.55, -0.5]} radius={0.02} castShadow receiveShadow>
